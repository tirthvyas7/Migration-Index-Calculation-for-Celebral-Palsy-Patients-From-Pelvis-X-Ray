...
#Initial Thresholding to segment out the bones
gray=cv2.cvtColor(imS,cv2.COLOR_BGR2GRAY)
blurred = cv2.GaussianBlur(gray, (9, 9), 2)
lower_th = 100 #The values are to be tuned for different images
upper_th = 150 #The values are to be tuned for different images
th_mask = cv2.inRange(blurred, lower_th, upper_th)
thresh = cv2.bitwise_and(imS, imS, mask=th_mask)

#Cropping for Calculation of A value
...
#Giving Option for Doctor to select the most preferable Corner from multiple detected corners
...
#Cropping for Calculation of B value
...
#Finding Contours in the Cropped Image
...
#Finding Maximum Area Bounding Box and Plotting on Image
